...
exports.StandardRealtimeStreamsManager = void 0;
...
const streamInstance_js_1 = require("./streamInstance.js");
class StandardRealtimeStreamsManager {
    ...
    pipe(key, source, options) {
        ...
        const readableStreamSource = (0, asyncIterableStream_js_1.ensureReadableStream)(source);
        ...
        const streamInstance = new streamInstance_js_1.StreamInstance({
...
exports.StandardRealtimeStreamsManager = StandardRealtimeStreamsManager;
function getRunIdForOptions(options) {
    if (options?.target) {
        if (options.target === "parent") {
            return task_context_api_js_1.taskContext.ctx?.run?.parentTaskRunId ?? task_context_api_js_1.taskContext.ctx?.run?.id;
        }
        if (options.target === "root") {
            return task_context_api_js_1.taskContext.ctx?.run?.rootTaskRunId ?? task_context_api_js_1.taskContext.ctx?.run?.id;
        }
