...
describe("nodeOptionsWithMaxOldSpaceSize", () => {
    ...
    it("replaces existing max-old-space-size flag", () => {
        const result = nodeOptionsWithMaxOldSpaceSize("--max-old-space-size=4096 --inspect", testMachine);
        expect(result).toBe(`--inspect ${expectedFlag}`);
    });
    it("handles multiple existing max-old-space-size flags", () => {
        const result = nodeOptionsWithMaxOldSpaceSize("--max-old-space-size=4096 --inspect --max-old-space-size=8192", testMachine);
        expect(result).toBe(`--inspect ${expectedFlag}`);
    });
